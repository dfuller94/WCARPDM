...
function MsgManageMessages(msg)
	...
	if LocalPlayer():GetInfoNum("rpdm_showmessages")==nil then
		CreateClientConVar("rpdm_showmessages", 1, true, false)
	end
	if LocalPlayer():GetInfoNum("rpdm_shownotify")==nil then
		CreateClientConVar("rpdm_shownotify", 0, true, false)
	end
	if LocalPlayer():GetInfoNum("rpdm_messages_warningnotify")==nil then
		CreateClientConVar("rpdm_messages_warningnotify", 1, true, false)
	end
	if LocalPlayer():GetInfoNum("rpdm_messages_dontshowincome")==nil then
		CreateClientConVar("rpdm_messages_dontshowincome", 0, true, false)
	end
	local mode = LocalPlayer():GetInfoNum("rpdm_showmessages")
	local both = LocalPlayer():GetInfoNum("rpdm_shownotify")
	local warn = LocalPlayer():GetInfoNum("rpdm_messages_warningnotify")
	local inc = LocalPlayer():GetInfoNum("rpdm_messages_dontshowincome")
