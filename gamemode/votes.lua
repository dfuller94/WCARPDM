...
function ccDoVote( ply, cmd, args )
	if( Votes[args[1]]==nil) then
		return
	end
	if( Votes[args[1]][tostring(ply:EntIndex()) .. "didvote"]==true ) then
		return
	end
	Votes[args[1]][tostring(ply:EntIndex()) .. "didvote"] = true
