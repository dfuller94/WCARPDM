...
function meta:IsOwned() 
	...
	if( self:GetNWInt( "Owner" ) ~= 0 or num > 0 ) then return true; end
...
function meta:GetDoorOwner()
	...
	return player.GetByID( self:GetNWInt( "Owner" ) ) or NULL;

end

function meta:IsMasterOwner( ply )

	if( ply:EntIndex() == self:GetNWInt( "Owner" ) ) then
		return true;
	end
...
function meta:OwnedBy( ply )
	...
	if( self:GetNWInt( "Owner" ) == ply:EntIndex() ) then return true; end
...
function meta:UnOwn( ply )
	...
	if( self:GetNWInt( "Owner" ) == 0 and num == 0 ) then
