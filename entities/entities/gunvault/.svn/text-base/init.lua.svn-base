...
function ENT:DropGun(gunnum, ply, hgt, bigdrop)
	...
	elseif (gun=="lasergun") then
		...
		weapon:SetUpgraded(upgrade)
		weapon:Spawn();
	elseif (gun=="lolbeam") then
		local weapon = ents.Create("spawned_weapon")
		weapon:SetModel( "models/weapons/w_irifle.mdl" );
		weapon:SetNWString("weaponclass", "weapon_lolbeam");
		weapon:SetPos( self.Entity:GetPos()+Vector(0,0,height));
		weapon:SetUpgraded(upgrade)
		weapon:Spawn();
...
function ENT:Touch(gun)
	if (gun:GetClass()=="spawned_weapon" && gun:GetNWString("weaponclass")!=nil && gun:GetNWString("weaponclass")!="weapon_physgun" && gun:GetNWString("weaponclass")!=""  && table.Count(self.Guns)<10 && gun:GetTime()<CurTime()-5) then
