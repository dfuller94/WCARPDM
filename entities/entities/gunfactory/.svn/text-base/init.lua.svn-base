...
function ENT:Initialize()
	...
	self.User = self.Entity:GetNWEntity("owner")
end
...
function ENT:CanProduce(guntype, ply)
	if guntype=="resetbutton" && ply==self.Entity:GetNWEntity("owner") then
		return true
...
function ENT:StartProduction(ply,guntype)
	local time = 120
	if guntype=="grenadegun" then time=180 elseif guntype=="worldslayer" then time = 600 elseif guntype=="plasma" then time = 180 end
	self.User = ply
...
function ENT:DoStuff()
	...
	if owner!=self.Entity:GetNWEntity("owner") then
		productioncost = 250
	end
