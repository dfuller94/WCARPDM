...
function ENT:Explode()
	...
	util.BlastDamage( self.Entity, self.Entity:GetNWEntity("owner"), self.Entity:GetPos(), 1536, 2000 )
	...
	for k, v in pairs(props) do 
		...
		if (class=="prop_physics_multiplayer" || class=="prop_physics_respawnable" || class=="prop_physics" || class=="phys_magnet" || class=="gmod_spawner" || class=="gmod_wheel" || class=="gmod_thruster" || class=="gmod_button" || class=="sent_keypad") then
	...
	for i=0, 15, 1 do
		...
		bomblet:SetNWEntity("owner", self.Entity:GetNWEntity("owner"))
	...
	local exp = ents.Create("env_physexplosion")
		...
		exp:GetTable().attacker = self.Entity:GetNWEntity("owner")
		exp:SetKeyValue("radius", 1536)
		exp:SetPos(pos)
	exp:Spawn()
	exp:SetOwner(self.Entity:GetNWEntity("owner"))
	exp:Fire("explode","",0)
	...
	if (self.Armed) then
		if (self.LastUsed+0.3>CurTime() && self.Disarming==60) then
			...
			if (self.Disarming<=0) then
				...
				Notify(self.Entity:GetNWEntity("owner"),1,3, "Bomb has been defused.")
...
function ENT:Armbomb(planter)
	...
	if (self.Entity:CheckPropFaggotry()==false) then
		...
		self.Arming = 50
		Notify(self.Entity:GetNWEntity("owner"), 4, 3, "Get your props away from the bomb for it to be usable.")
...
function ENT:CheckPropFaggotry()
	...
	for k, v in pairs(ents.FindInSphere( self.Entity:GetPos(), 1536) ) do
		if (player.GetByUniqueID(v:GetVar("PropProtection"))!=false) then
			local entowner = player.GetByUniqueID(v:GetVar("PropProtection"))
			if self.Entity:GetNWEntity("owner")==entowner then
...
function ENT:PropCheck()
	if (self.Entity:CheckPropFaggotry()==true) then
		Notify(self.Entity:GetNWEntity("owner"),1,3, "Your props are too close! Bomb has self-defused.")
		self.Entity:EmitSound(Sound("weapons/c4/c4_disarm.wav"))
		self.Entity:Remove()
	end
end
...
function ENT:FindBreakables()
	...
	for k, v in pairs(props) do
		local class = v:GetClass()
		if (class=="prop_physics_multiplayer" || class=="prop_physics_respawnable" || class=="prop_ragdoll"  || class=="prop_physics" || class=="phys_magnet" || class=="gmod_spawner" || class=="gmod_wheel" || class=="gmod_thruster" || class=="gmod_button") then
