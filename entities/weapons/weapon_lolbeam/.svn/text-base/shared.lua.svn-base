...
if ( CLIENT ) then
	...
	killicon.AddFont( "weapon_lolbeam", "HL2MPTypeDeath", SWEP.IconLetter, Color( 100, 100, 100, 255 ) )
...
function SWEP:PrimaryAttack()
	...
	self:CSShootBullet( 1, 0, self.Primary.NumShots, 0 )
	...
	local effectdata = EffectData()
		...
		effectdata:SetAngle(Angle(tonumber(self.Owner:GetInfo("rpdm_clcolor_r")),tonumber(self.Owner:GetInfo("rpdm_clcolor_g")),tonumber(self.Owner:GetInfo("rpdm_clcolor_b"))))
	util.Effect("laserbeam", effectdata)
	
	if SERVER && ValidEntity(beamstart.Entity) && beamstart.Entity:IsPlayer() then
		local ent = beamstart.Entity
		BurnPlayer(ent,1,self.Owner,self.Weapon)
		StunPlayer(ent,3,self.Owner,self.Weapon)
		PoisonPlayer(ent,1,self.Owner,self.Weapon)
	end
